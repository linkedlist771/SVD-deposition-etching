...
from eval.utils.inception import InceptionV3
...
class FIDConfig(BaseModel):
    batch_size: int = Field(default=50, description="Batch size to use")
    num_workers: Optional[int] = Field(
        default=None,
        description="Number of processes to use for data loading. Defaults to min(8, num_cpus)",
    )
    device: Optional[str] = Field(
        default=None, description="Device to use. Like cuda, cuda:0 or cpu"
    )
    dims: int = Field(
        default=2048,
        description="Dimensionality of Inception features to use. By default, uses pool3 features",
    )
...
def calculate_fid(config: FIDConfig):
    ...
    if config.save_stats:
        save_fid_stats(
            config.paths, config.batch_size, device, config.dims, num_workers
        )
        return
