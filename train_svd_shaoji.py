...
from utils.infer_utils import export_to_gif, generate_video_from_image
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="The duration(s) of the generated image"
    )
...
def main():
    ...
    # add validation calculation
    # Create test image directory from validation dataset
    test_image_dir = os.path.join(args.output_dir, "test_images")
    os.makedirs(test_image_dir, exist_ok=True)

    # Number of test samples to extract
    num_test_samples = min(len(val_dataset), args.num_validation_images)  # Extract up to 10 samples

    # Randomly sample test images from validation dataset
    test_image_indices = random.sample(range(len(val_dataset)), num_test_samples)
    for i, idx in enumerate(test_image_indices):
        sample = val_dataset[idx]
        img_tensor = sample["pixel_values"][0]  # Extract the first frame as the test image
        img_array = ((img_tensor + 1) * 127.5).clamp(0, 255).numpy().astype(np.uint8)
        img = Image.fromarray(np.transpose(img_array, (1, 2, 0)))  # Convert to HWC format
        img.save(os.path.join(test_image_dir, f"test_image_{i}.png"))

    # Update test_images to list images from test_image_dir
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                conditional_pixel_values = pixel_values[
                    :, 0:1, :, :, :
                ]  #  这行代码是在从输入的视频帧张量中提取第一帧作为条件帧， 也就是作为输入。
                    ...
                    if (global_step % args.validation_steps == 0) or (global_step == 1):
                        ...
                        for image_name in test_images:
                            ...
                            generate_video_from_image(
                                ...
                                duration_sec=args.generation_duration,
                                height=args.height,
                                width=args.width
                            )
                        if args.use_ema:
