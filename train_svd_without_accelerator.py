...
def _resize_with_antialiasing(input, size, interpolation="bicubic", align_corners=True):
    ...
    if (ks[1] % 2) == 0:
        ks = ks[0], ks[1] + 1
    input = _gaussian_blur2d(input, ks, torch.tensor([sigmas], device=input.device))
...
def main():
    ...
    if args.non_ema_revision is not None:
        deprecate(
            ...
            "0.15.0",
            message=("Downloading 'non_ema' weights from revision branches is deprecated."),
        )
    ...
    # Learning rate scaling (not strictly needed in single GPU mode)
    if args.scale_lr:
        # Just keep as is, no distributed.
    ...
    if args.resume_from_checkpoint:
        ...
        if os.path.isdir(checkpoint_path):
            ...
            model_files = [f for f in checkpoint_files if f.endswith(".pt")]
            if len(model_files) > 0:
                model_file = os.path.join(checkpoint_path, model_files[0])
                ...
                if args.use_ema and "ema_unet" in checkpoint:
                    ema_unet.load_state_dict(checkpoint["ema_unet"])
                print(f"Resumed from checkpoint {model_file}")
        else:
            print(f"Checkpoint {args.resume_from_checkpoint} does not exist. Starting fresh.")
    ...
    def _get_add_time_ids(
        ...
        if expected_add_embed_dim != passed_add_embed_dim:
            raise ValueError(
                f"Model expects an added time embedding vector of length {expected_add_embed_dim}, got {passed_add_embed_dim}."
            )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            sigmas = rand_log_normal(shape=[bsz], loc=0.7, scale=1.6, device=device)
            sigmas = sigmas[:, None, None, None, None]
            noisy_latents = latents + noise * sigmas
            timesteps = torch.Tensor([0.25 * sigma.log() for sigma in sigmas.squeeze()]).to(device)
            ...
            loss = torch.mean(
                (weighing.float() * (denoised_latents.float() - target.float()) ** 2).reshape(target.shape[0], -1), dim=1
            ).mean()
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.checkpointing_steps == 0:
                    ...
                    torch.save(ckpt, os.path.join(save_path, "model.pt"))
                ...
                if (global_step % args.validation_steps == 0) or (global_step == 1):
                    ...
                    with torch.no_grad():
                        for val_step, val_batch in enumerate(val_dataloader):
                            ...
                            noise = torch.randn_like(latents)
                            bsz = latents.shape[0]
                            cond_sigmas = rand_log_normal(shape=[bsz], loc=-3.0, scale=0.5, device=device)
                            noise_aug_strength = cond_sigmas[0]
                            cond_sigmas = cond_sigmas[:, None, None, None, None]
